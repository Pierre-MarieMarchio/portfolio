...
describe('object camera', () => {
  describe('measureHome', () => {
    ...
    it('lays the system down as the free band narrows', () => {
      const tall = measureHome({ width: 1280, height: 1000 }, 72, 56);
      const low = measureHome({ width: 1280, height: 420 }, 72, 56);
      expect(low.ev).toBeLessThan(tall.ev);
      expect(Math.abs(low.i)).toBeLessThan(Math.abs(tall.i));
    });
  });

  describe('sheetFrame and previewFrame', () => {
    ...
    it('never frames the preview smaller than home, nor outside its bounds', () => {
      for (const cardLeft of [null, 0, 300, 9000]) {
        ...
        expect(frame.s).toBeGreaterThanOrEqual(HOME_FRAME.s * 1.25);
