...
export const drawConstellations = (
  ...
  const hidden = (x: number, y: number): boolean =>
    ...
    Math.sqrt((x - hole.cx) * (x - hole.cx) + (y - hole.cy) * (y - hole.cy)) <
      hole.R * 1.05;
  ctx.lineCap = 'round';
  CONSTELLATIONS.forEach((figure, k) => {
    const on = args.lit[k] ?? 0;
    const a = (0.2 + 0.8 * on) * shown;
    if (a < 0.02) {
      return;
    }
    const size = Math.min(w, h) * figure.t;
    const ox = w * figure.x - args.panX * 0.55 * w * depth + drift;
    const oy = h * figure.y - args.panY * 0.55 * h * depth;
    const points = figure.pts.map(
      ([px, py]) => [ox + (px - 0.5) * size, oy + (py - 0.5) * size] as const,
    );
    ctx.globalAlpha = a * (0.16 + 0.26 * on) * entry;
    ...
    for (const [i, j] of figure.lines) {
      const p = points[i];
      const q = points[j];
      if (!p || !q || hidden(p[0], p[1]) || hidden(q[0], q[1])) {
        continue;
      }
      ctx.moveTo(p[0], p[1]);
      ctx.lineTo(q[0], q[1]);
    ...
    ctx.globalAlpha = on * shown * 0.9 * entry;
    ctx.fillStyle = '#ffffff';
