...
import {
  ...
  repel,
  ScreenPoint,
} from '../../rules/planets/planet-spacing.rules';
...
export class PlanetsRenderer {
  ...
  private shape(
    ...
    body.rising = vn;
    body.coverFade = focus.isOverview ? 1 : frame.veil(planet.sx, planet.sy);
    body.isCold = i >= focus.faintFrom;
  ...
  private drawBody(body: PlanetBody, frame: SceneFrame): void {
    ...
    ctx.stroke();
    if (isRinged(frame.focus, body.rank)) {
