 ...
 */
export type ObjectPanelRole = '' | 'head' | 'rule' | 'sheet' | 'preview';
 ...
 * planets. They sign in through `appObjectPanel` and `appObjectLine`, so the
...
export class ObjectRegistry {
  private readonly panelSet = new Set<ObjectPanel>();
  private readonly lineSet = new Set<HTMLElement>();

  public addPanel(panel: ObjectPanel): () => void {
  ...
  public panels(): readonly ObjectPanel[] {
